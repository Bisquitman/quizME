...
const renderTheme = (themes) => {
  ...
  const buttons = themes.map((theme) => {
    ...
    if (result) {
      ...
      p.innerHTML = `
        <span class="selection__result-ratio">${result}/${theme.list.length}</span>
...
// Перемешивание ответов по методу Фишера-Йетса
const shuffle = array => {
...
const createAnswer = data => {
  ...
  const labels = answersList.map((item, i) => {
...
const renderQuiz = (quiz) => {
  ...
  const showQuestion = () => {
    ...
    fieldset.append(legend, ...answersData.labels); // ...answers - то же самое, что и answers[0], answers[1], ... , answers[n], где n заранее неизвестно
